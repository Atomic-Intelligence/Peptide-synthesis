...
def main(cfg: DictConfig):
    ...
    if run_dcr:
        ...
        dcr_estimator = DCREstimator(
            ...
            holdout_fraction=(
                dcr_cfg.get("holdout_fraction", 0.5)
                if isinstance(dcr_cfg, dict)
                else getattr(dcr_cfg, "holdout_fraction", 0.5)
            ),
            par_percentile=(
                dcr_cfg.get("par_percentile", 5.0)
                if isinstance(dcr_cfg, dict)
                else getattr(dcr_cfg, "par_percentile", 5.0)
            ),
            distance_metric=(
                dcr_cfg.get("distance_metric", "euclidean")
                if isinstance(dcr_cfg, dict)
                else getattr(dcr_cfg, "distance_metric", "euclidean")
            ),
        )
    ...
    if run_reid:
        ...
        reid = ReidentificationRiskEstimator(
            ...
            categorical_columns=CATEGORICAL_CLINICAL_COLUMNS + ["event_type"],
            risk_threshold=(
                reid_cfg.get("risk_threshold", 2.0)
                if isinstance(reid_cfg, dict)
                else getattr(reid_cfg, "risk_threshold", 2.0)
            ),
            distance_metric=(
                reid_cfg.get("distance_metric", "euclidean")
                if isinstance(reid_cfg, dict)
                else getattr(reid_cfg, "distance_metric", "euclidean")
            ),
        )
        reid.fit(real_data)
