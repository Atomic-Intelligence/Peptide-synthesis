...
def merge_hf_data(root_dir_path: str, save_dir_path: str):
    ...
    excel_data = pd.ExcelFile(root_dir_path + "AnalysisIDs_List_Synthetic_patients_final.xlsx")
    filenames_data = pd.read_excel(excel_data, sheet_name="list.datasets")
    filenames_data = filenames_data.drop(filenames_data.columns[[4, 5, 6, 7, 8, 9]], axis=1)
    ...
    hf_filenames = filenames_data[filenames_data["Group"] == "HF"]["File name"].unique()
    hf_peptides_data_merged = concatenate_tables_from_files(root_dir_path, hf_filenames)
    ...
    no_event_clinical_path = os.path.join(save_dir_path, "no_event_clinical_data.csv")
    ...
    no_event_peptides_path = os.path.join(save_dir_path, "no_event_peptides_data.csv")
