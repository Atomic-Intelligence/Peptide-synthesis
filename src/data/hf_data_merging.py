...
def merge_hf_data(root_dir_path: str, save_dir_path: str):
    ...
    if Path(root_dir_path).exists() and any(Path(root_dir_path).iterdir()):
        ...
        excel_data = pd.ExcelFile(
            root_dir_path + "AnalysisIDs_List_Synthetic_patients_final_INPUT file_V2.xlsx"
        )
        ...
        hf_clin_data = hf_clin_data.drop(["HF.event", "CAD.event", "CKD.event", "time-to-CKDevent(to event or last visit)"], axis=1)
        ...
        # Process CKD clinical data and save ckd ids
        print("CKD clinical data processing and saving")
        ckd_clin_data = clin_data[clin_data["CKD.event"] == 1].reset_index(drop=True)
        ckd_clin_data = ckd_clin_data.drop(["CKD.event", "CAD.event", "HF.event"], axis=1)
        ckd_ids = list(ckd_clin_data.idAuswertung)
        ckd_ids = [id * 1000 for id in ckd_ids]

        # save CKD clinical data
        Path(save_dir_path).mkdir(exist_ok=True, parents=True)
        ckd_save_path = os.path.join(save_dir_path, "ckd_clinical_data.csv")
        ckd_clin_data.to_csv(ckd_save_path, index=False)

        # Process no-event clinical data
        print("no-event clinical data processing and saving")
        no_event_clin_data = clin_data[
            (clin_data["CAD.event"] == 0) & (clin_data["HF.event"] == 0) & (clin_data["CKD.event"] == 0)
        ].reset_index(drop=True)
        no_event_clin_data = no_event_clin_data.drop(["HF.event", "CAD.event", "CKD.event"], axis=1)
        ...
        # Process no-event peptide data (CKD peptide data is a subset of non-event peptide data)
        print("no-event and cdk peptide data processing and saving")
        ...
        # divide peptide data into no-event and CKD
        ckd_peptide = df_ne_reset[df_ne_reset.idAuswertung.isin(ckd_ids)]
        no_event_peptides = df_ne_reset[~df_ne_reset.idAuswertung.isin(ckd_ids)]

        # save no-event peptide data
        no_event_peptides_path = os.path.join(
            save_dir_path, "no_event_peptides_data.csv"
        )
        no_event_peptides.to_csv(no_event_peptides_path, index=False)

        # save ckd peptide data
        ckd_peptides_path = os.path.join(
            save_dir_path, "ckd_peptides_data.csv"
        )
        ckd_peptide.to_csv(ckd_peptides_path, index=False)
